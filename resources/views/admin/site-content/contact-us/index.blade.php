    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-normal" id="addModalLabel">New Select Item</h5>
...
<div class="modal fade" id="modalEdit" tabindex="-1" role="dialog" aria-labelledby="modalEdit"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-normal" id="editModalLabel">Edit Select Item</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<div class="modal fade" id="modalDelete" tabindex="-1" role="dialog" aria-labelledby="modalDelete"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-normal" id="deleteModalLabel">Delete Item</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<div class="py-4">
    ...
    @if (Request::is('admin/contact-us'))
    <form action="{{ route('admin.contact-us.update', $siteContent) }}" method="post" id="frmProductDetail" enctype="multipart/form-data">
        @csrf
        @method('PUT')
    ...
    @if (!Request::is('admin/request-demo'))
    @foreach ($siteContent->childs as $item)
    @if ($item->slug == $pathurl)
    <div class="row mb-4">
        <div class="col">
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <button class="btn bg-gradient-dark btn-md" data-bs-toggle="modal"
                            ...
                            onclick="itemAdd({{$item}})">{{ 'Add New' }}</button>
                    </div>
                </div>
                <div class="card-body pt-0 pb-2">
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            ...
                            <tbody>
                                @forelse ($item->contentMetas as $key => $selectItem)
                                <tr>
                                    <td>
                                        <div class="d-flex px-2 py-1">
                                            <div class="d-flex gap-4 justify-content-center">
                                                ...
                                                <div>
                                                    <h6 class="mb-0 text-sm">{{ $selectItem->value }}</h6>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="align-middle text-end">
                                        @if (count($item->contentMetas) > 0)
                                        <a 
                                            href="#"
                                            class="text-secondary font-weight-bold text-xs act-btn me-3"
                                            data-bs-toggle="modal" data-bs-target="#modalDelete"
                                            onclick="itemDelete({{$selectItem}}, {{$item}})">
                                            Delete
                                        </a>
                                        @endif
                                        ...
                                        <a 
                                            ...
                                            onclick="itemEdit({{$selectItem}}, {{$item}})">
                                            Edit
                                        </a>
                                    </td>
                                </tr>
...
<script>
    ...
    function itemAdd(item) {
        ...
        if (modalLabel) {
            modalLabel.innerText = 'New ' + item.title + ' Item'
        }
    }

    function itemEdit(item, parent) {
        ...
        let modalLabel = document.getElementById('editModalLabel');
        if (modalLabel) {
            modalLabel.innerText = 'Edit ' + parent.title + ' Item'
        }
    }

    function itemDelete(item, parent) {
        ...
        if (modalLabel) {
            modalLabel.innerText = 'Delete ' + parent.title + ' Item'
        }
    }
