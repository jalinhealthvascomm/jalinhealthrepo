    ...
    <div class="col">
        <div class="navbar navbar-expand-lg navbar-light bg-white z-index-3 py-3">
            <div class="ps-0 d-flex justify-content-between" style="width: 100%">
                ...
                <h5 class="mb-0">Update User - {{ $user->name }}</h5>
            </div>
        </div>
    </div>
</div>
...
<div class="row mb-4">
    <div class="col">
        <form action="{{ route('admin.users-list.update', $user) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control" placeholder="Name" name="name" id="name"
                            aria-label="Name" aria-describedby="name" value="{{ $user->name }}">
                        @error('name')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="email" class="form-control" placeholder="Email" name="email" id="email"
                            aria-label="Email" aria-describedby="email-addon" value="{{ $user->email }}" readonly>
                        @error('email')
                        <p class="text-danger text-xs mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group mb-3">
                                <label for="password" style="margin-left: 0!important;">
                    ...
                    //<div class="row">
                    //     <div class="col">
                    //         <select class="role-select form-select form-select-lg mb-3" aria-label=".form-select-lg example" name="role" @disabled(true)>
                    //             {{-- <option selected>Open this select menu</option> --}}
                    //             @forelse ($availableRoles as $item)
                    //                 <option value="{{ $item }}" @if ($item == $user->getRoleNames()[0]) selected @endif>{{ $item }}</option>
                    //             @empty
